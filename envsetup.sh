...
function check_product()
{
    ...
    INV_PRODUCT=$(echo -n $1 | sed 's/_.*//g')
...
function breakfast()
{
    target=$1
    local variant=$2
    INV_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/invictus/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Invictus model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch inv_$target-$variant
        fi
    fi
    return $?
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $INV_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.invictus.device | grep -q "$INV_BUILD");
    then
        ...
        echo "The connected device does not appear to be $INV_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.invictus.device | grep -q "$INV_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/invictus/build/envsetup.sh
